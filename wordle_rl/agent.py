...
class RNNAgent(nn.Module):
    ...
    def forward(self, letter_seq, state_seq):
        """
            ...
            outputs:
                "actions": (batch_size x action_length)
        ...
        if self.debug_mode:
            fig, ax = plt.subplots(1, self.output_len, figsize=(self.output_len * 3, 1 * 3))
        ...
        for t in range(self.output_len):
            ...
            if self.debug_mode:
                ...
                sns.heatmap(map_reshaped, linewidth=0.5, ax=ax[t], cmap='rocket')
    ...
    def act(self, inputs):
        """
            ...
            outputs:
                "actions": (batch_size x action_length)
                "log_probs": None (non-applicable to this agent)
                "values": (batch_size)
        """
...
class RandomAgent():
    def __init__(self, game_voc_matrix):
        super().__init__()
        self.game_voc_matrix = game_voc_matrix
    ...
    def act(self, inputs):
        ...
        idx = torch.randint(high=len(self.game_voc_matrix), size=(batch_size,))
        actions = self.game_voc_matrix[idx]
